#!/usr/bin/env python3
"""
Script to detect new-grad job opportunities from README.md changes
...
def parse_opportunities_from_readme_lines(lines: List[str]) -> List[Dict[str, Any]]:
    """Parse job opportunities from README table rows."""
    opportunities = []
    ...
    while i < len(lines):
        ...
        # Look for table rows that contain job data
        if '<tr>' in line:
            ...
            # Check if this row contains a job opportunity
...
def parse_single_opportunity_from_row(row_text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        role = role_match.group(1).strip() if role_match else "New Grad Role"
        ...
        return {
            ...
            'terms': ['New Grad'],  # New-grad positions repo
            'age': age
        }
...
def find_new_opportunities_from_patches() -> List[Dict[str, Any]]:
    """Find new-grad opportunities by analyzing README patch files."""
    new_grad_opportunities = []
    ...
    for patch_file in patch_files:
        ...
        if added_lines:
            ...
            new_grad_opportunities.extend(new_opportunities)

    # De-duplicate by company name and URL
    deduped: List[Dict[str, Any]] = []
    seen_keys: Set[Tuple[str, str]] = set()
    for opp in new_grad_opportunities:
...
def generate_email_body(new_opportunities: List[Dict[str, Any]]) -> str:
    ...
    if not new_opportunities:
        return """📭 NO NEW GRAD JOBS DETECTED

No new-grad job opportunities (0d age) were found in the latest sync.
Check back later for new opportunities!"""

    # Header
    email_content = """🚀 NEW GRAD JOB OPPORTUNITIES DETECTED! 🚀

The following NEW new-grad jobs (just added today - 0d age) have been found:
...
"""
    ...
    # Footer
    email_content += """💡 TIP: Apply early! These new-grad opportunities were just added today (0d age) and competition is high.
...
def main():
    ...
    print("🔍 Analyzing patch files for new-grad opportunities (0d age only)...")

    # Find new-grad opportunities from patch files
    new_opportunities = find_new_opportunities_from_patches()
    ...
    print(f"✅ Found {len(new_opportunities)} new-grad opportunities (0d age)")
    print("📧 Email body written to email/body.txt")

    # Print summary for debugging
    if new_opportunities:
        print("\n🎯 New new-grad opportunities found (0d age):")
        ...
        print("📭 No new new-grad opportunities (0d age) found in the changes")
