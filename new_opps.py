...
# --- Email theme (new-grad = blue) ---------------------------------------
ACCENT = "#2563eb"
EMOJI = "🎓"
LABEL_SHORT = "new-grad"
...
Check back later for new opportunities!"""
    ...
    lines = [f"🎓 {count} new grad {noun} just posted (added today).", ""]
...
def generate_email_html(new_opportunities: List[Dict[str, Any]]) -> str:
    ...
    noun = "role" if count == 1 else "roles"
    if not new_opportunities:
        header = "No new grad roles right now"
        sub = "Nothing new in the latest sync — check back soon."
        cards = ""
    else:
        header = f"{EMOJI} {count} new grad {noun} just posted"
